...
export function flowReducer( state, action ) {
    switch (action.type) {
        case "SWITCH_MODE": {
            // flip between login and signInWithPopup, restteing back to the first step.
...
export function friendlyError(err) {
    switch(err?.code) {
        ...
        case "auth/invalid-email":
            return "That email is invalid.";
        case "auth/popup-closed-by-user":
            return "The sign-in window was closed before finishing."
        case "auth/operation-not-allowed":
            return "That sign-in method is not enabled i nyour Firebase project yet.";
        default:
